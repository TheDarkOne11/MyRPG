...
void GameScreen::update() {
	offsetY = player->getY() - Handler::getHeight()/2;
	offsetX = player->getX() - Handler::getWidth()/2;
}
...
bool GameScreen::isInScreen(const int y, const int x) const {
	...
	if(screenY >= 0 && screenX >= 0) {
		if(screenY <= Handler::getHeight() && screenX <= Handler::getWidth()) {
			return true;
		}
	}
...
void GameScreen::paint(const std::vector<std::vector<MyObject*> >& vect_levelMap) {
	...
	int startY = player->getY() - Handler::getHeight()/2 - offsetY;
	// Y coordinate of bottom-right corner
	int endY = player->getY() + Handler::getHeight()/2 - offsetY;
	...
	for(int y = startY; y < (signed) vect_levelMap.size(); y++) {
		int startX = player->getX() - Handler::getWidth()/2 - offsetX;
		int endX = player->getX() + Handler::getWidth()/2 - offsetX;
